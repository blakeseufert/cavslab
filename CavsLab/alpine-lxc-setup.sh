...
${NC}"
echo -e  "${YELLOW}--- Alpine LXC setup script. Includes Docker, Tailscale & Utils ---${NC}" 
echo -e "${YELLOW}------ Connects to cephfs share /dockerdata/${NC}"                                                                          
                                                                           
# Update package lists and install all required utilities and services.
echo -e "${YELLOW}Updating package list and installing packages...${NC}" 
apk update
apk add docker || true
...
echo -e "${YELLOW}Enabling services to start at boot (docker, tailscale, sshd)...${NC}" 
...
echo -e "${YELLOW}Setup Tailscale tun userspace-networking...${NC}" 
/usr/sbin/tailscaled --tun=userspace-networking
echo -e "${YELLOW}Starting Tailscale interactive setup...${NC}" 
tailscale up
